...
//
// Usage: npm run check:calendar
const esbuild = require('esbuild');
const path = require('path');
const Module = require('module');

const result = esbuild.buildSync({
  entryPoints: [path.resolve(__dirname, '../src/lib/reviewCalendar.projection.check.ts')],
  bundle: true,
  platform: 'node',
  format: 'cjs',
  write: false,
...
const m = new Module('calendar-check', null);
m.filename = path.resolve(__dirname, 'calendar-check-bundle.cjs');
