      ...
      <ScrollView className="flex-1 bg-white" contentContainerClassName="p-5" contentContainerStyle={{ gap: 16 }}>
        ...
        <View className="flex-row items-center gap-3">
          ...
          <View>
            ...
            <AppText variant="title" className="font-serif font-bold text-[#1A1A1A]">{selectedBook}</AppText>
          </View>
        </View>
        ...
        <View className="flex-1 pt-3">
          {!bookData ? (
            <AppText variant="label" className="text-neutral-400 ">No chapters.</AppText>
          ) : (
            <View className="flex-row flex-wrap gap-3">
              {Array.from({ length: bookData.chapters }, (_, i) => i + 1).map((chNum) => (
                ...
                >
                  <AppText variant="title" className="text-[#1A1A1A] font-serif font-bold ">{chNum}</AppText>
                </Pressable>
              ))}
            </View>
          )}
