import { VerseState, Recording, MemoryPlan, GroupPlan } from './types';

export const BOOKS = {
  ot: [
    { name: 'Genesis', chapters: [1, 2, 3] },
    { name: 'Psalms', chapters: [23, 119] }
  ],
  nt: [
    { name: 'John', chapters: [1, 15] },
    { name: 'Romans', chapters: [8, 12] }
  ]
};

export const INITIAL_VERSES: VerseState[] = [
  // Genesis 1
  { book: 'Genesis', chapter: 1, verse: 1, text: 'In the beginning, God created the heavens and the earth.', status: 'memorized', dueDate: 'Completed' },
  { book: 'Genesis', chapter: 1, verse: 2, text: 'The earth was without form and void, and darkness was over the face of the deep.', status: 'memorized', dueDate: 'Completed' },
  { book: 'Genesis', chapter: 1, verse: 3, text: 'And God said, "Let there be light," and there was light.', status: 'learning', dueDate: 'Due: Today' },
  { book: 'Genesis', chapter: 1, verse: 4, text: 'And God saw that the light was good. And God separated the light from the darkness.', status: 'learning', dueDate: 'Due: Today' },
  { book: 'Genesis', chapter: 1, verse: 5, text: 'God called the light Day, and the darkness he called Night. And there was evening and there was morning, the first day.', status: 'learning', dueDate: 'Due: Today' },
  { book: 'Genesis', chapter: 1, verse: 6, text: 'And God said, "Let there be an expanse in the midst of the waters, and let it separate the waters from the waters."', status: 'learning', dueDate: 'Due: Today' },
  { book: 'Genesis', chapter: 1, verse: 7, text: 'And God made the expanse and separated the waters that were under the expanse from the waters that were over the expanse. And it was so.', status: 'untouched' },
  { book: 'Genesis', chapter: 1, verse: 8, text: 'And God called the expanse Heaven. And there was evening and there was morning, the second day.', status: 'untouched' },
  { book: 'Genesis', chapter: 1, verse: 9, text: 'And God said, "Let the waters under the heavens be gathered together into one place, and let the dry land appear." And it was so.', status: 'untouched' },
  { book: 'Genesis', chapter: 1, verse: 10, text: 'God called the dry land Earth, and the waters that were gathered together he called Seas. And God saw that it was good.', status: 'untouched' },

  // Genesis 2
  { book: 'Genesis', chapter: 2, verse: 1, text: 'Thus the heavens and the earth were finished, and all the host of them.', status: 'untouched' },
  { book: 'Genesis', chapter: 2, verse: 2, text: 'And on the seventh day God finished his work that he had done, and he rested on the seventh day from all his work that he had done.', status: 'untouched' },
  { book: 'Genesis', chapter: 2, verse: 3, text: 'So God blessed the seventh day and made it holy, because on it God rested from all his work that he had done in creation.', status: 'untouched' },

  // Psalms 23
  { book: 'Psalms', chapter: 23, verse: 1, text: 'The Lord is my shepherd; I shall not want.', status: 'memorized', dueDate: 'Completed' },
  { book: 'Psalms', chapter: 23, verse: 2, text: 'He makes me lie down in green pastures. He leads me beside still waters.', status: 'memorized', dueDate: 'Completed' },
  { book: 'Psalms', chapter: 23, verse: 3, text: 'He restores my soul. He leads me in paths of righteousness for his name\'s sake.', status: 'learning', dueDate: 'Due: Tomorrow' },
  { book: 'Psalms', chapter: 23, verse: 4, text: 'Even though I walk through the valley of the shadow of death, I will fear no evil, for you are with me; your rod and your staff, they comfort me.', status: 'learning', dueDate: 'Due: Tomorrow' },
  { book: 'Psalms', chapter: 23, verse: 5, text: 'You prepare a table before me in the presence of my enemies; you anoint my head with oil; my cup overflows.', status: 'untouched' },
  { book: 'Psalms', chapter: 23, verse: 6, text: 'Surely goodness and mercy shall follow me all the days of my life, and I shall dwell in the house of the Lord forever.', status: 'untouched' },

  // John 15
  { book: 'John', chapter: 15, verse: 20, text: 'Remember the word that I said to you: "A servant is not greater than his master." If they persecuted me, they will also persecute you. If they kept my word, they will also keep yours.', status: 'learning', dueDate: 'Due: Today' },
  { book: 'John', chapter: 15, verse: 21, text: 'But all these things they will do to you on account of my name, because they do not know him who sent me.', status: 'learning', dueDate: 'Due: Today' },
  { book: 'John', chapter: 15, verse: 22, text: 'If I had not come and spoken to them, they would have no excuse for their sin.', status: 'untouched' },

  // Romans 8
  { book: 'Romans', chapter: 8, verse: 1, text: 'There is therefore now no condemnation for those who are in Christ Jesus.', status: 'memorized', dueDate: 'Completed' },
  { book: 'Romans', chapter: 8, verse: 2, text: 'For the law of the Spirit of life has set you free in Christ Jesus from the law of sin and death.', status: 'memorized', dueDate: 'Completed' },
  { book: 'Romans', chapter: 8, verse: 3, text: 'For God has done what the law, weakened by the flesh, could not do. By sending his own Son in the likeness of sinful flesh and for sin, he condemned sin in the flesh,', status: 'learning', dueDate: 'Due: Today' },
  { book: 'Romans', chapter: 8, verse: 4, text: 'in order that the righteous requirement of the law might be fulfilled in us, who walk not according to the flesh but according to the Spirit.', status: 'learning', dueDate: 'Due: Today' },
  { book: 'Romans', chapter: 8, verse: 5, text: 'For those who live according to the flesh set their minds on the things of the flesh, but those who live according to the Spirit set their minds on the things of the Spirit.', status: 'learning', dueDate: 'Due: Today' },
  { book: 'Romans', chapter: 8, verse: 6, text: 'For to set the mind on the flesh is death, but to set the mind on the Spirit is life and peace.', status: 'learning', dueDate: 'Due: Today' },
  { book: 'Romans', chapter: 8, verse: 7, text: 'For the mind that is set on the flesh is hostile to God, for it does not submit to God\'s law; indeed, it cannot.', status: 'learning', dueDate: 'Due: Today' },
  { book: 'Romans', chapter: 8, verse: 8, text: 'Those who are in the flesh cannot please God.', status: 'learning', dueDate: 'Due: Today' },
  { book: 'Romans', chapter: 8, verse: 9, text: 'You, however, are not in the flesh but in the Spirit, if in fact the Spirit of God dwells in you. Anyone who does not have the Spirit of Christ does not belong to him.', status: 'learning', dueDate: 'Due: Today' },
  { book: 'Romans', chapter: 8, verse: 10, text: 'But if Christ is in you, although the body is dead because of sin, the Spirit is life because of righteousness.', status: 'learning', dueDate: 'Due: Today' },
];

export const INITIAL_RECORDINGS: Recording[] = [
  {
    id: 'rec_1',
    title: 'Romans 8 Chapter Recitation',
    book: 'Romans',
    chapter: 8,
    translation: 'ESV',
    duration: 180,
    date: '2026-06-20',
    user: 'Kenneth Carter',
    avatar: 'K',
    category: 'group',
    versesStr: 'Full Chapter'
  },
  {
    id: 'rec_2',
    title: 'Psalms 23 Chapter Recitation',
    book: 'Psalms',
    chapter: 23,
    translation: 'NKJV',
    duration: 120,
    date: '2026-06-22',
    user: 'Kenneth Carter',
    avatar: 'K',
    category: 'group',
    versesStr: 'Full Chapter'
  }
];

export const SUGGESTED_FEED_RECORDINGS: Recording[] = [
  {
    id: 'feed_1',
    title: 'Romans 8 Full Chapter Reading',
    book: 'Romans',
    chapter: 8,
    translation: 'ESV',
    duration: 195,
    date: '2026-06-24',
    user: 'Sarah Miller',
    avatar: 'SM',
    category: 'group',
    versesStr: 'Full Chapter'
  },
  {
    id: 'feed_2',
    title: 'Psalms 23 Full Chapter Recital',
    book: 'Psalms',
    chapter: 23,
    translation: 'NKJV',
    duration: 90,
    date: '2026-06-23',
    user: 'Elizabeth K.',
    avatar: 'EK',
    category: 'friends',
    versesStr: 'Full Chapter'
  },
  {
    id: 'feed_3',
    title: 'Genesis 1 Full Chapter Recital',
    book: 'Genesis',
    chapter: 1,
    translation: 'ESV',
    duration: 155,
    date: '2026-06-22',
    user: 'Brother Thomas',
    avatar: 'BT',
    category: 'group',
    versesStr: 'Full Chapter'
  },
  {
    id: 'feed_4',
    title: 'Romans 12 Full Chapter Recital',
    book: 'Romans',
    chapter: 12,
    translation: 'NIV',
    duration: 140,
    date: '2026-06-25',
    user: 'Mark Davis',
    avatar: 'MD',
    category: 'group',
    versesStr: 'Full Chapter'
  },
  {
    id: 'feed_5',
    title: 'John 15 Full Chapter Recital',
    book: 'John',
    chapter: 15,
    translation: 'NIV',
    duration: 165,
    date: '2026-06-21',
    user: 'Pastor Robert',
    avatar: 'PR',
    category: 'global',
    versesStr: 'Full Chapter'
  },
  {
    id: 'feed_6',
    title: 'Genesis 2 Full Chapter Recital',
    book: 'Genesis',
    chapter: 2,
    translation: 'NASB',
    duration: 130,
    date: '2026-06-19',
    user: 'Grace Thompson',
    avatar: 'GT',
    category: 'global',
    versesStr: 'Full Chapter'
  }
];

export const DEFAULT_PLANS: MemoryPlan[] = [
  {
    id: 'genesis-foundations',
    name: 'Genesis — Foundations Track',
    preset: 'custom',
    learningDays: ['M', 'T', 'W', 'Th', 'F'],
    reviewingDays: ['M', 'T', 'W', 'Th', 'F', 'S', 'Su'],
    primingDays: ['W', 'F'],
    newVersesPace: 3,
    maxReviewCap: 15,
    isActive: true,
    updatedAt: new Date().toISOString()
  },
  {
    id: 'warrior-track',
    name: 'Warrior Track',
    preset: 'warrior',
    learningDays: ['M', 'T', 'W', 'Th', 'F', 'S'],
    reviewingDays: ['M', 'T', 'W', 'Th', 'F', 'S', 'Su'],
    primingDays: ['T', 'Th', 'S'],
    newVersesPace: 5,
    maxReviewCap: 30,
    isActive: false,
    updatedAt: new Date().toISOString()
  },
  {
    id: 'gentle-drip',
    name: 'Gentle Drip',
    preset: 'drip',
    learningDays: ['M', 'W'],
    reviewingDays: ['M', 'W', 'F'],
    primingDays: ['F'],
    newVersesPace: 1,
    maxReviewCap: 10,
    isActive: false,
    updatedAt: new Date().toISOString()
  }
];

export const MOCK_GROUP_PLANS: GroupPlan[] = [
  {
    planId: 'group-plan-1',
    circleId: 'adult-bible-study',
    name: 'Romans 8 Foundation',
    managerId: 'mock-manager-1',
    managerName: 'Pastor David',
    scriptureRange: ['ROM_8_1', 'ROM_8_2', 'ROM_8_3', 'ROM_8_4', 'ROM_8_5', 'ROM_8_6', 'ROM_8_7', 'ROM_8_8', 'ROM_8_9', 'ROM_8_10'],
    startDate: new Date(Date.now() - 10 * 24 * 3600 * 1000).toISOString(),
    pacingPerWeek: 3,
    learningDays: ['Mon', 'Wed', 'Fri'],
    currentGroupVerseIndex: 5,
    description: 'A study group focusing on the first ten verses of the powerhouse chapter, Romans 8.'
  },
  {
    planId: 'group-plan-2',
    circleId: 'youth-sprints',
    name: 'Genesis 1 Foundations',
    managerId: 'mock-manager-2',
    managerName: 'Esther Vance',
    scriptureRange: ['GEN_1_1', 'GEN_1_2', 'GEN_1_3', 'GEN_1_4', 'GEN_1_5', 'GEN_1_6', 'GEN_1_7', 'GEN_1_8', 'GEN_1_9', 'GEN_1_10'],
    startDate: new Date(Date.now() - 4 * 24 * 3600 * 1000).toISOString(),
    pacingPerWeek: 2,
    learningDays: ['Tue', 'Thu'],
    currentGroupVerseIndex: 3,
    description: 'Mastering the creation account in Genesis chapter 1.'
  }
];

export const DUMMY_PROFILES: Record<string, any> = {
  'Sarah Miller': {
    name: 'Sarah Miller',
    avatar: 'SM',
    level: 'Memory Level: Devoted Scholar',
    planName: 'Warrior Pacing Routine',
    preset: 'warrior',
    newVersesPace: 3,
    maxReviewCap: 15,
    learningDays: ['M', 'W', 'F', 'S'],
    reviewingDays: ['M', 'T', 'W', 'Th', 'F', 'S', 'Su'],
    primingDays: ['T', 'Th', 'Su'],
    stats: { memorized: 32, learning: 8, streak: 32 },
    communities: ['Adult Bible Study Group', 'Early Bird Romans Study'],
    friends: ['Thomas Wright', 'Chloe Vance', 'Kenneth Carter'],
    recordings: [
      { id: 'sm_1', book: 'Romans', chapter: 8, translation: 'ESV', duration: 195, date: '2026-06-24' },
      { id: 'sm_2', book: 'John', chapter: 15, translation: 'NIV', duration: 165, date: '2026-06-21' }
    ],
    activityGrid: [
      { day: 'June 10', count: 4 },
      { day: 'June 11', count: 6 },
      { day: 'June 12', count: 3 },
      { day: 'June 13', count: 8 },
      { day: 'June 14', count: 5 },
      { day: 'June 15', count: 2 },
      { day: 'June 16', count: 0 },
      { day: 'June 17', count: 9 },
      { day: 'June 18', count: 11 },
      { day: 'June 19', count: 4 },
      { day: 'June 20', count: 7 },
      { day: 'June 21', count: 6 },
      { day: 'June 22', count: 5 },
      { day: 'June 23', count: 8 },
      { day: 'June 24', count: 12 },
    ]
  },
  'Thomas Wright': {
    name: 'Thomas Wright',
    avatar: 'TW',
    level: 'Memory Level: Daily Pacer',
    planName: 'Gentle Word Drip',
    preset: 'drip',
    newVersesPace: 1,
    maxReviewCap: 10,
    learningDays: ['M', 'Th'],
    reviewingDays: ['M', 'W', 'F'],
    primingDays: ['S'],
    stats: { memorized: 18, learning: 5, streak: 12 },
    communities: ['Adult Bible Study Group'],
    friends: ['Sarah Miller', 'Pastor David', 'Kenneth Carter'],
    recordings: [
      { id: 'tw_1', book: 'Genesis', chapter: 1, translation: 'ESV', duration: 155, date: '2026-06-22' },
      { id: 'tw_2', book: 'Genesis', chapter: 2, translation: 'NASB', duration: 130, date: '2026-06-19' }
    ],
    activityGrid: [
      { day: 'June 10', count: 1 },
      { day: 'June 11', count: 2 },
      { day: 'June 12', count: 0 },
      { day: 'June 13', count: 1 },
      { day: 'June 14', count: 2 },
      { day: 'June 15', count: 1 },
      { day: 'June 16', count: 0 },
      { day: 'June 17', count: 2 },
      { day: 'June 18', count: 3 },
      { day: 'June 19', count: 1 },
      { day: 'June 20', count: 0 },
      { day: 'June 21', count: 2 },
      { day: 'June 22', count: 4 },
      { day: 'June 23', count: 1 },
      { day: 'June 24', count: 2 },
    ]
  },
  'Chloe Vance': {
    name: 'Chloe Vance',
    avatar: 'CV',
    level: 'Memory Level: Rising Disciple',
    planName: 'The 30-Day Scripture Sprint',
    preset: 'sprint',
    newVersesPace: 5,
    maxReviewCap: 20,
    learningDays: ['M', 'T', 'W', 'Th', 'F'],
    reviewingDays: ['M', 'T', 'W', 'Th', 'F', 'S', 'Su'],
    primingDays: ['S', 'Su'],
    stats: { memorized: 15, learning: 12, streak: 9 },
    communities: ['Adult Bible Study Group', 'Youth Scripture Sprints'],
    friends: ['Esther Vance', 'Sarah Miller', 'Kenneth Carter'],
    recordings: [
      { id: 'cv_1', book: 'Psalms', chapter: 23, translation: 'NKJV', duration: 90, date: '2026-06-23' }
    ],
    activityGrid: [
      { day: 'June 10', count: 5 },
      { day: 'June 11', count: 8 },
      { day: 'June 12', count: 0 },
      { day: 'June 13', count: 3 },
      { day: 'June 14', count: 4 },
      { day: 'June 15', count: 9 },
      { day: 'June 16', count: 0 },
      { day: 'June 17', count: 2 },
      { day: 'June 18', count: 7 },
      { day: 'June 19', count: 4 },
      { day: 'June 20', count: 12 },
      { day: 'June 21', count: 0 },
      { day: 'June 22', count: 6 },
      { day: 'June 23', count: 5 },
      { day: 'June 24', count: 8 },
    ]
  },
  'Esther Vance': {
    name: 'Esther Vance',
    avatar: 'EV',
    level: 'Memory Level: Scripture Champion',
    planName: 'The 30-Day Scripture Sprint',
    preset: 'sprint',
    newVersesPace: 5,
    maxReviewCap: 20,
    learningDays: ['M', 'T', 'W', 'Th', 'F'],
    reviewingDays: ['M', 'T', 'W', 'Th', 'F', 'S', 'Su'],
    primingDays: ['S', 'Su'],
    stats: { memorized: 54, learning: 10, streak: 28 },
    communities: ['Adult Bible Study Group', 'Youth Scripture Sprints'],
    friends: ['Chloe Vance', 'Pastor David', 'Kenneth Carter'],
    recordings: [
      { id: 'ev_1', book: 'John', chapter: 15, translation: 'NIV', duration: 175, date: '2026-06-25' },
      { id: 'ev_2', book: 'Romans', chapter: 8, translation: 'ESV', duration: 185, date: '2026-06-20' }
    ],
    activityGrid: [
      { day: 'June 10', count: 12 },
      { day: 'June 11', count: 15 },
      { day: 'June 12', count: 8 },
      { day: 'June 13', count: 14 },
      { day: 'June 14', count: 10 },
      { day: 'June 15', count: 16 },
      { day: 'June 16', count: 4 },
      { day: 'June 17', count: 11 },
      { day: 'June 18', count: 15 },
      { day: 'June 19', count: 12 },
      { day: 'June 20', count: 18 },
      { day: 'June 21', count: 6 },
      { day: 'June 22', count: 14 },
      { day: 'June 23', count: 15 },
      { day: 'June 24', count: 20 },
    ]
  },
  'Pastor David': {
    name: 'Pastor David',
    avatar: 'PD',
    level: 'Memory Level: Circle Elder',
    planName: 'The 30-Day Scripture Sprint',
    preset: 'sprint',
    newVersesPace: 5,
    maxReviewCap: 20,
    learningDays: ['M', 'T', 'W', 'Th', 'F'],
    reviewingDays: ['M', 'T', 'W', 'Th', 'F', 'S', 'Su'],
    primingDays: ['S', 'Su'],
    stats: { memorized: 112, learning: 4, streak: 120 },
    communities: ['Adult Bible Study Group', 'Early Bird Romans Study', 'Youth Scripture Sprints'],
    friends: ['Thomas Wright', 'Esther Vance', 'Kenneth Carter'],
    recordings: [
      { id: 'pd_1', book: 'Romans', chapter: 8, translation: 'ESV', duration: 190, date: '2026-06-18' }
    ],
    activityGrid: [
      { day: 'June 10', count: 10 },
      { day: 'June 11', count: 12 },
      { day: 'June 12', count: 14 },
      { day: 'June 13', count: 15 },
      { day: 'June 14', count: 8 },
      { day: 'June 15', count: 11 },
      { day: 'June 16', count: 12 },
      { day: 'June 17', count: 10 },
      { day: 'June 18', count: 14 },
      { day: 'June 19', count: 11 },
      { day: 'June 20', count: 15 },
      { day: 'June 21', count: 12 },
      { day: 'June 22', count: 13 },
      { day: 'June 23', count: 15 },
      { day: 'June 24', count: 14 },
    ]
  }
};

export const getProfileForName = (name: string): any => {
  if (DUMMY_PROFILES[name]) {
    return DUMMY_PROFILES[name];
  }
  const cleanName = name || 'Anonymous Disciple';
  const firstLetter = cleanName[0].toUpperCase();
  return {
    name: cleanName,
    avatar: firstLetter,
    level: 'Memory Level: Committed Learner',
    planName: 'Gentle Word Drip',
    preset: 'drip',
    newVersesPace: 2,
    maxReviewCap: 12,
    learningDays: ['M', 'W', 'F'],
    reviewingDays: ['M', 'W', 'F', 'S'],
    primingDays: ['Su'],
    stats: { memorized: 12, learning: 4, streak: 7 },
    recordings: [
      { id: `fall_${cleanName}`, book: 'Romans', chapter: 8, translation: 'ESV', duration: 145, date: '2026-06-23' }
    ],
    activityGrid: [
      { day: 'June 10', count: 1 },
      { day: 'June 11', count: 2 },
      { day: 'June 12', count: 0 },
      { day: 'June 13', count: 1 },
      { day: 'June 14', count: 2 },
      { day: 'June 15', count: 1 },
      { day: 'June 16', count: 0 },
      { day: 'June 17', count: 2 },
      { day: 'June 18', count: 3 },
      { day: 'June 19', count: 1 },
      { day: 'June 20', count: 0 },
      { day: 'June 21', count: 2 },
      { day: 'June 22', count: 4 },
      { day: 'June 23', count: 1 },
      { day: 'June 24', count: 2 },
    ]
  };
};
